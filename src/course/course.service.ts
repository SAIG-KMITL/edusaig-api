...
import { UpdateCourseDto, CreateCourseDto, PaginatedCourseResponeDto } from "./dtos/index";
...
export class CourseService {
    ...
    async update(
        ...
        this.validateStatusTransition(existingCourse.status, updateCourseDto.status);
    ...
    private buildWhereCondition(
        ...
        const conditions: Record<Role, () => FindOptionsWhere<Course> | FindOptionsWhere<Course>[]> = {
            [Role.STUDENT]: () => ({
                ...baseCondition,
        ...
        switch (role) {
            ...
            case Role.ADMIN:
                if (course.status !== CourseStatus.DRAFT) {
                    throw new BadRequestException('Admin can only update courses in draft status');
                }
                break;
    ...
    private validateStatusTransition(currentStatus: CourseStatus, newStatus?: CourseStatus): void {
        if (!newStatus) return;

        if (currentStatus !== CourseStatus.DRAFT && newStatus === CourseStatus.DRAFT) {
