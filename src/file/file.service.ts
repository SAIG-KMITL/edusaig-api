...
export class FileService {
    ...
    async upload(folder: Folder, key: string, file: Express.Multer.File): Promise<void> {
        try {
            await this.s3Client.send(
                new PutObjectCommand({
                    Bucket: this.configService.getOrThrow<string>(GLOBAL_CONFIG.AWS_BUCKET_NAME),
    ...
    async update(folder: Folder, key: string, file: Express.Multer.File): Promise<void> {
